...
function google_api() {
    ...
    if (neighborhood_object[address]) {
        ...
        currentCentroid = _.find(scene.children, function(mesh) {
            return mesh.name == currentRollover;
        }).properties.centroid;
        census_latlon = null;
                            ...
                            $.ajax({
                                ...
                                success: function(data) {
                                    ...
                                    if (data !== "") {
                                        ...
                                        census_latlon = [lat,lon];
                                        onDocumentClick(data, 1);
                                       $("#validAddress").css({"visibility":"hidden"});
                                    ...
                                    // reset the search value
                                        ...
                                        input.value = "";
                                        // error message -- needs to try again
                                        $("#validAddress").html("PLEASE ENTER A VALID ADDRESS")
                                        TweenLite.to($('#validAddress'), .5, {
                                            autoAlpha: 1,
                                            delay: .375
                                        });
                                        census_latlon = null;
